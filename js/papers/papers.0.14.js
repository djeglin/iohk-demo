...
function io_papers_move_update(){
	...
	$("#papers_row").css({width:(paper+20)*(papers.length)});
	...
	if(moved_papers){
		...
		if(bw > 767){
			...
			//$("#papers .papers").scrollLeft( 300 );
			$("#papers .papers").scrollLeft(active * (paper + 20) - ((bw-paper)/2));
			//			document.getElementById('papers_row').scrollTo(500,0);
