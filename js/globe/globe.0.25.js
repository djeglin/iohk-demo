...
var io_globe_autoRotate = false;
var io_globe_autoRotate_count = 0;
var io_globe_autoRotate_intset = false;
...
DAT.Globe = function(container, opts) {
  ...
  var globemap = '/images/world-new-b.jpg';
  var globevar = 'black';
  var globeatmo = ' 0.3, 0.3, 0.3, 1.0 ';
  globevar = document.getElementById("io_data_globe").getAttribute("data-var");
  if(globevar == 'white'){
    globemap = '/images/globe-ww.jpg';
    globeatmo = ' 0.9, 0.9, 0.9, 1.0 ';
  }
    ...
    'atmosphere' : {
      ...
      fragmentShader: [
        ...
        'void main() {',
          ...
          'gl_FragColor = vec4('+globeatmo+') * intensity;',
        '}'
      ].join('\n')
    }
  };
  ...
  function init() {
    ...
    uniforms['texture'].value = my_TextureLoader.load(globemap);
...
//io_globe_autoRotate_intset

  function io_startAutoRotate() {

    if(io_globe_autoRotate){
    ...
    if(locIndex == new_coords.length){
      ...
      if(!io_globe_autoRotate_intset){
        io_globe_autoRotate_intset = true;
        io_globe_autoRotate_count = setInterval(io_startAutoRotate, 4000);
      }
  ...
  if(io_globe_type == 'multi'){
    ...
    if(io_globe_autoRotate_intset){
      io_globe_var = null;
      io_globe_autoRotate = false;
      io_globe_autoRotate_intset = false;
      clearInterval(io_globe_autoRotate_count);
    }


    io_globe_autoRotate_count = 0;
    arr_deselect('io_data_globe_profile');
    io_load_data();
    //arr_select(0,'io_data_globe_profile');
    io_globe_autoRotate = true;
    io_startAutoRotate();
    setTimeout(function(){
      jQuery("#io_data_globe-box").addClass('loaded');
      setTimeout(function(){
