...
function io_zotero_item_details(dataload) {
    ...
    <div class="date">'+date+''+io_nohttp(publisher)+' &nbsp; <small class="dateAdded"><em class="fa fa-paperclip"></em> Added on '+io_zotero_date(dataload.data.dateAdded,'','. ',' ','')+'</small></div>\
    '+conferenceName+'\
    '+libraryCatalog+'\
    '+extra+'\
...
function io_zotero_search_load() {
	if(!io_zotero_loaded_full){
		$.getJSON('https://api.zotero.org/groups/'+io_zotero_group+'/items?start='+io_zotero_search_loaded+'&limit='+(io_zotero_search_loaded+io_zotero_search_step)+'&format=json&search=website-&v=1&key='+io_zotero_pubkey, function (data) {
	    	if(data.length > 0){
				for(var i=0;i<data.length;i++){
...
function io_zotero_search() {
	if(!io_zotero_search_init){
		...
		$('#library-search,.library_autocomplete').click(function(event){
...
/*
	...
	if(out != ''){
		if(io_zotero_search_autocomplete){
			$(".library_autocomplete").html('<ul>'+out+'</ul>');
		}else{
	   		$("#zotero_load_details").empty();
			$(".library_autocomplete").empty();
	    	$("#zotero_load_details").html('<div class="library-title"><h2 class="margint0"><em class="icon fa fa-search searched rounded"></em> Search results for "'+searched+'"</h2></div>');
