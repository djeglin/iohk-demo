	...
	if(dataload.data.creators != undefined){
		for(var c=0;c<dataload.data.creators.length;c++){
			if(dataload.data.creators[c].firstName != undefined){
				...
				var creator = io_get_member_link(dataload.data.creators[c].name,true);
				if(creator){
					creators += '<li>'+creator+'</li>';
				}
			}
		}
	}
				...
				for(var i=0;i<data.length;i++){
					if(data[i].data.filename != undefined){
						...
						iframes += '\
							...
							<div class="icons none">'+icons+'</div>\
							</div>\
							';
					}
...
function io_zotero_library_item(dataload) {
	...
	var item = '\
    <div class="book is-collapsed '+io_zotero_tag_papers(dataload.data.tags)+' '+active+'">\
        <div class="text">\
