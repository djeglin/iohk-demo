...
<div class="row">\
<div class="col-sm-12">\
<div class="btn-group inline-block" role="group" aria-label="View controls">\
...
var zotero_current_sort = 'dateAdded';
...
zotero_sorting.date = 'Publish Date';
...
library_tools = '\
	...
	<div class="dropdown inline-block">\
	  <button class="btn btn-default dropdown-toggle btn-lg" type="button" id="lib_sort_by" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" title="Sort items by ...">\
	    Sort by <span class="lab">'+zotero_sorting[zotero_current_sort]+'</span>\
	    <span class="caret"></span>\
	  </button>\
	  <ul class="dropdown-menu" aria-labelledby="lib_sort_by">\
	...
	<div class="dropdown inline-block">\
	  <button class="btn btn-default dropdown-toggle btn-lg" type="button" id="lib_order" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">\
	    <span class="lab">'+zotero_order[zotero_current_order]+'</span> Order\
	    <span class="caret"></span>\
	  </button>\
	...
	<div class="btn-group" role="group" aria-label="View controls">\
	    ...
	    <div class="btn-group" role="group">\
		    ...
		    <ul class="dropdown-menu">\
		        <li><a href="http://twitter.com/share?url='+url+'&text='+title+'&via=InputOutput" target="_blank">'+iolab_share_on_Twitter+'</a></li>\
			    <li><a href="http://www.facebook.com/sharer.php?u='+url+'&t='+title+'" target="_blank">'+iolab_share_on_Facebook+'</a></li>\
			    <li><a href="http://www.linkedin.com/shareArticle?mini=true&url='+url+'" target="_blank">'+iolab_share_on_LinkedIn+'</a></li>\
			    <li><a href="https://plusone.google.com/_/+1/confirm?hl=en&url='+url+'" target="_blank">'+iolab_share_on_Google_Plus+'</a></li>\
			    <li><a href="http://reddit.com/submit?url='+url+'&title='+title+'" target="_blank">'+iolab_share_on_Reddit+'</a></li>\
		    </ul>\
		</div>\
	</div>\
	';
...
function io_zotero_item_details(dataload) {
	...
	if(dataload.data.url != undefined && dataload.data.url != ''){
		url = '<div class="url"><a class="btn btn-primary rounded c_f00" href="'+dataload.data.url+'" target="_blank">Link &nbsp;<em class="fa fa-external-link"></em></a></div>';
	}
	var publisher = '';
    ...
    <div class="date">'+date+''+io_nohttp(publisher)+', <small><em class="fa fa-paperclip"></em> Added on '+io_zotero_date(dataload.data.dateAdded,'','. ',' ','')+'</small></div>\
    '+conferenceName+'\
    '+libraryCatalog+'\
    '+extra+'\
...
function io_zotero_item_html(dataload) {
	...
	var th = jQuery('.abstractNote .text').height();
	if(th > 200){
		jQuery('.abstractNote').addClass('hiding');
		jQuery('.abstractNote .grad,.abstractNote .button_posts').show();
	}else{
		jQuery('.abstractNote .grad,.abstractNote .button_posts').hide();
	}
}
...
function io_zotero_item_deselect() {
	jQuery('.is-expanded').removeClass('is-expanded').addClass('is-collapsed');
}


function io_zotero_item_preview_note() {
	var th = jQuery('.is-expanded .abstractNote .text').height();
	if(th > 200){
		jQuery('.is-expanded .abstractNote').addClass('hiding');
		jQuery('.is-expanded .abstractNote .grad,.is-expanded .abstractNote .button_posts').show();
	}else{
		jQuery('.is-expanded .abstractNote .grad,.is-expanded .abstractNote .button_posts').hide();
	}
}
...
function io_zotero_item_click(obj) {
	...
	if (thisCell.hasClass('is-collapsed')) {
		jQuery("#library .is-expanded").removeClass('is-expanded').addClass('is-collapsed');
	...
	//setTimeout(function(){
		//if(hash == window.location.hash){
			...
			if(io_zotero_layout != 'item'){
				jQuery(".is-expanded .zotero_details_wrap").html(io_zotero_item_details(io_zotero_arr_obj[io_lib_item_selected]));
...
function io_zotero_item() {
	jQuery(".library_autocomplete").empty();	
	jQuery("#zotero_loading").addClass('loading');
	jQuery("#zotero_details").empty();
        ...
        <div class="text">\
            <div class="open_book" id="book-'+dataload.key+'" rel="'+dataload.key+'" onclick="io_zotero_item_click(this)">\
            	...
            	<div class="bg">\
                    ...
                    <div class="dateAdded" title="Added on '+io_zotero_date(dataload.data.dateAdded,'','. ',' ','')+'"><b class="rounded"><em class="fa fa-paperclip"></em> '+io_zotero_date(dataload.data.dateAdded,'','. ',' ','')+'</b></div>\
...
function io_zotero_lib_list_setup() {
	...
	if(!tag){
    	...
    	jQuery("#zotero_load_details").html('<div class="library-title"><h2 class="margint0"><em class="icon fa fa-tag website-'+tag+' rounded"></em> '+tag+'</h2></div>');
    	jQuery("#zotero_load").html(io_zotero_library());
...
function io_zotero_load(tag,callback) {
	...
	//console.log(io_zotero_lib_list_tag);
	//alert(io_zotero_loaded_full);
	jQuery("#rightcolwrap").attr('class','');
	...
	if(io_zotero_loaded_full){
		...
		jQuery.getJSON('https://api.zotero.org/groups/'+io_zotero_group+'/items?start='+io_zotero_lib_list_loaded+'&limit='+(io_zotero_lib_list_loaded+io_zotero_lib_list_step)+'&sort=dateAdded&format=json'+tag_val+'&v=3&key='+io_zotero_pubkey, function (data) {
	    	if(data){
				for(var i=0;i<data.length;i++){
					if(io_zotero_tag_ok(data[i].data.tags)){
						io_zotero_arr_obj[data[i].key] = new Object();
...
function io_zotero_search() {
	if(!io_zotero_search_init){
		...
		jQuery('#library-search,.library_autocomplete').click(function(event){
		    event.stopPropagation();
		});
		io_zotero_search_init = true;
	}
	jQuery(".library_autocomplete.none").removeClass('none');	
}
...
/*
						...
						if (io_zotero_arr_obj[key].data.tags[i].tag.match('website-')) {
							...
							if(searching_tags[i] != -1){
								...
								if(io_zotero_search_autocomplete){
									...
									tagline += '<span class="label tag '+doDashes(io_zotero_arr_obj[key].data.tags[i].tag)+'"><a href="#tag-'+doDashes(tag_s)+'"><em class="fa fa-tag"></em> '+tag_s.substr(0,searching_tags[i])+'<b>'+tag_s.substr(searching_tags[i],searched.length)+'</b>'+tag_s.substr(searching_tags[i]+searched.length,tag_s.length)+'</a></span>';
								}
							}else{
								if(io_zotero_search_autocomplete){
									tagline += '<span class="tag">'+tag_s+'</span> ';
								}else{
									tagline += '<span class="label tag '+doDashes(io_zotero_arr_obj[key].data.tags[i].tag)+'"><a href="#tag-'+doDashes(tag_s)+'"><em class="fa fa-tag"></em> '+tag_s+'</a></span>';
								}
							}
						}
					}
				...
				if(str){
					...
					if(!io_zotero_search_autocomplete){
						out += abstractNote;
						out += '<a class="btn btn-default rounded readmore" href="#'+io_zotero_arr_obj[key].key+'">'+iolab_read_more+' &nbsp;<em class="fa fa-chevron-right"></em></a>';
					}
	...
	if(out != ''){
		if(io_zotero_search_autocomplete){
			jQuery(".library_autocomplete").html('<ul>'+out+'</ul>');
		}else{
	   		jQuery("#zotero_load_details").empty();
			jQuery(".library_autocomplete").empty();
	    	jQuery("#zotero_load_details").html('<div class="library-title"><h2 class="margint0"><em class="icon fa fa-search searched rounded"></em> Search results for "'+searched+'"</h2></div>');
	    	jQuery("#zotero_load").html('<ul class="results">'+out+'</ul>');
		}
	}
}
