	...
	if(io_zotero_arr_obj[io_zotero_selection].icons == undefined){
		jQuery.getJSON('https://api.zotero.org/groups/'+io_zotero_group+'/items/'+io_zotero_selection+'/children?key='+io_zotero_pubkey+'&format=json&include=data', function (data) {
	    	if(data){
    			...
    			jQuery("#zotero_load_detail").html('<div class="col-lg-offset-3"><h1 class="margint0">'+io_zotero_arr_obj[io_zotero_selection].data.title+'</h1></div>'+iframes).removeClass('none');
	    	}
		});
	}
	return false;
...
function io_zotero_load(tag,callback) {
	...
	//alert(tag);searches
	jQuery.getJSON('https://api.zotero.org/groups/'+io_zotero_group+'/items?start=0&limit=100&format=json&search=website-&v=1&key='+io_zotero_pubkey, function (data) {
    	if(data){
    		console.log(data);
    		io_zotero_arr = [];
			for(var i=0;i<data.length;i++){
...
function io_zotero_tag_load(callback) {
	var lib_list = true;
	var tag_val = '';
	if(tag != ''){
		tag_val = '&tag=website-'+tag+'';
