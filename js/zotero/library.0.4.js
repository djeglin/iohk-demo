	...
	if(searched.length > 0){
		...
		for (var key in io_zotero_arr_obj) {
			if(io_zotero_arr_obj[key].data.title != undefined){ ///io_zotero_item_creators(dataload)
				...
				if(str){
					res += '<li class="tags">';
					res += '<a href="#'+io_zotero_arr_obj[key].key+'">';
					if(tit != ''){
						res += tit;
					}else{
						res += '<span class="block title">'+io_zotero_arr_obj[key].data.title+'</span>';
					}
					if(aut != ''){
						res += aut;
					}else{
						res += '<span class="block authors"><em class="fa fa-graduation-cap"></em> '+io_strip(io_zotero_item_creators(io_zotero_arr_obj[key]))+'</span>';
					}
					res += '<span class="block tags uppercase"><em class="fa fa-tag"></em> '+tagline+'</span>';
					res += '</a>';
					res += '</li>';
				}
