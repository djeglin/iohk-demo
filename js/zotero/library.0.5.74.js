...
var library_tools = function(){
	...
	<div class="dropdown inline-block">\
	  ...
	  <ul class="dropdown-menu" aria-labelledby="lib_sort_by">\
	    <li><a href="javascript:;" onclick="io_zotero_reorder(this)" rel="sort" val="dateAdded">Date Added</a></li>\
	    <li><a href="javascript:;" onclick="io_zotero_reorder(this)" rel="sort" val="date">Publish Date</a></li>\
	    <li><a href="javascript:;" onclick="io_zotero_reorder(this)" rel="sort" val="title">Title</a></li>\
	    <li><a href="javascript:;" onclick="io_zotero_reorder(this)" rel="sort" val="publisher">Publisher</a></li>\
	  </ul>\
	</div> &nbsp; \
	<div class="dropdown inline-block">\
...
function io_zotero_item_html(dataload) {
	jQuery("#zotero_load_preview").empty();
...
function io_zotero_item_click(obj) {
	...
	if(io_zotero_layout != 'item'){
		$("#zotero_load_preview").html('<div class="wrap"><div class="inner">'+io_zotero_item_details(io_zotero_arr_obj[io_lib_item_selected])+'</div></div>');
...
function io_zotero_reorder(obj) {
	var changing = jQuery(obj).attr('rel');
	var val = jQuery(obj).attr('val');
	if(changing == 'sort'){
		...
		jQuery(obj).parent().parent().prev().find('.lab').text(zotero_sorting[zotero_current_sort]);
	}else{
		zotero_current_order = val;
		jQuery(obj).parent().parent().prev().find('.lab').text(zotero_order[zotero_current_order]);
	}
...
function io_zotero_load(tag,callback) {
	...
	console.log(zotero_current_sort);
	...
	if(io_zotero_loaded_full){
		...
		jQuery.getJSON('https://api.zotero.org/groups/'+io_zotero_group+'/items?sort='+zotero_current_sort+'&direction='+zotero_current_order+'&start='+io_zotero_lib_list_loaded+'&limit='+(io_zotero_lib_list_step)+'&format=json'+tag_val+'&v=3&key='+io_zotero_pubkey, function (data) {
	    	if(data){
				for(var i=0;i<data.length;i++){
					if(io_zotero_tag_ok(data[i].data.tags)){
						io_zotero_arr_obj[data[i].key] = new Object();
