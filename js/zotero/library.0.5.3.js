...
function io_zotero_load(tag,callback) {
	...
	if(!io_zotero_loaded_full){
		...
		$.getJSON('https://api.zotero.org/groups/'+io_zotero_group+'/items?start='+io_zotero_lib_list_loaded+'&limit='+(io_zotero_lib_list_loaded+io_zotero_lib_list_step)+'&format=json'+tag_val+'&v=1&key='+io_zotero_pubkey, function (data) {
	    	if(data){
				for(var i=0;i<data.length;i++){
					if(io_zotero_tag_ok(data[i].data.tags)){
						io_zotero_arr_obj[data[i].key] = new Object();
