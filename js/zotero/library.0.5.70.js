...
function io_zotero_item_details(dataload) {
	...
	if(io_zotero_layout == 'item'){
		out += '<a href="javascript:io_zotero_item_back_button()" class="back rounded"><em class="fa fa-chevron-left"></em><span>'+iolab_back+'</span></a> <h1 class="margint0 roboto"> <a href="'+dataload.data.url+'" target="_blank">'+dataload.data.title+' <sup><em class="fa fa-external-link"></em></sup></a></h1>';
	}else{
    ...
    '+ISBN+'\
    '+abstractNote+'\
    <div class="row buttons nomobile uppercase"><div class="col-sm-12 col-xs-12"><a class="btn btn-primary" href="#'+dataload.key+'">'+iolab_read_more+' &nbsp;<em class="fa fa-chevron-right"></em></a></div><div class="col-sm-12 col-xs-12 text-right"><a class="btn btn-default" href="javascript:io_zotero_item_deselect()">Close</a></div></div>\
    <div class="row buttons nodesktop"><hr><div class="col-sm-12 col-xs-11"><a class="btn btn-default" href="javascript:io_zotero_item_deselect()">'+iolab_close+' &nbsp;<em class="fa fa-times"></em></a></div><div class="col-sm-12 col-xs-13 text-right"><a class="btn btn-default btn-primary" href="#'+dataload.key+'">'+iolab_read_more+' &nbsp;<em class="fa fa-chevron-right"></em></a></div></div>\
	';
	if(io_zotero_layout == 'item'){
		out = '\
				...
				for(var i=0;i<data.length;i++){
					if(data[i].data.filename != undefined){
						...
						iframes += '\
							...
							<iframe id="'+cls+'" src="https://api.zotero.org/groups/'+io_zotero_group+'/items/'+attach_id+'/file/view?key='+io_zotero_pubkey+'" width="100%" height="1160" style="border:0;border-bottom:1px solid #555">Loading ...</iframe>\
							</div>\
