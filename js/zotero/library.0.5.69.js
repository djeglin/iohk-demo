...
function io_zotero_item_details(dataload) {
    ...
    <div class="nomobile url">'+url+'</div>\
    '+place+'\
    '+ISBN+'\
    '+abstractNote+'\
				...
				for(var i=0;i<data.length;i++){
					if(data[i].data.filename != undefined){
						...
						iframes += '\
							...
							<iframe id="'+cls+'" src="https://api.zotero.org/groups/'+io_zotero_group+'/items/'+attach_id+'/file/view?key='+io_zotero_pubkey+'" width="100%" height="1150" style="border:0">Loading ...</iframe>\
							</div>\
	...
	setTimeout(function(){
		if(hash == window.location.hash){
			...
			if(io_zotero_layout != 'item'){
				...
				//jQuery("#rightcolwrap").attr('class','library_item_selected');
			}
		}
