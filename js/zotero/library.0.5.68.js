...
function io_zotero_item_details(dataload) {
    ...
    '+ISBN+'\
    '+abstractNote+'\
    <div class="row buttons"><div class="col-sm-14 col-xs-14 pdr0 nomobile"><a class="btn btn-default btn-block rounded" href="#'+dataload.key+'">'+iolab_read_more+' &nbsp;<em class="fa fa-chevron-right"></em></a></div><div class="col-sm-10 col-xs-10 ">'+url+'</div></div>\
	    	...
	    	if(data){
				...
				for(var i=0;i<data.length;i++){
					if(data[i].data.filename != undefined){
						...
						iframes += '<iframe id="'+cls+'" src="https://api.zotero.org/groups/'+io_zotero_group+'/items/'+attach_id+'/file/view?key='+io_zotero_pubkey+'" width="100%" height="1000" style="border:0">Loading ...</iframe>';
...
function io_zotero_item_click(obj) {
	...
	setTimeout(function(){
		if(hash == window.location.hash){
			...
			if(io_zotero_layout != 'item'){
				jQuery("#zotero_details").html('<div class="wrap"><div class="inner">'+io_zotero_item_details(io_zotero_arr_obj[io_lib_item_selected])+'</div></div>');
    ...
    <div class="book">\
        <div class="text">\
            <div class="open_book" id="book-'+dataload.key+'" rel="'+dataload.key+'" onclick="io_zotero_item_click(this)">\
            	...
            	<div class="bg">\
                    <div class="opener text-center"><a href="#'+dataload.key+'" class="rounded uppercase">'+iolab_open+'</a></div>\
