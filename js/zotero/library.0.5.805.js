...
var library_item_tools = function(){
	...
	<div class="btn-group" role="group" aria-label="View controls">\
	    <a type="button" class="btn btn-default btn-print" onclick="window.print()"><em class="fa fa-print"></em></a>\
	    <a type="button" class="btn btn-default" data-title="'+title+'" data-link="'+url+'" onclick="io_send_mail(this)"><em class="fa fa-envelope"></em></a>\
	    <a href="http://twitter.com/share?url='+url+'&text='+title+'&via=InputOutput" class="btn btn-default" target="_blank"><em class="fa fa-twitter"></em></a>\
	    <a href="http://www.facebook.com/sharer.php?u='+url+'&t='+title+'" class="btn btn-default" target="_blank"><em class="fa fa-facebook"></em></a>\
	    <a href="http://www.linkedin.com/shareArticle?mini=true&url='+url+'" class="btn btn-default" target="_blank"><em class="fa fa-linkedin"></em></a>\
	    <a href="https://plusone.google.com/_/+1/confirm?hl=en&url='+url+'" class="btn btn-default" target="_blank"><em class="fa fa-google-plus"></em></a>\
	    <a href="http://reddit.com/submit?url='+url+'&title='+title+'" class="btn btn-default" target="_blank"><em class="fa fa-reddit"></em></a>\
	</div>\
	';
	return out;
}
...
function io_get_member(nam){
	var out = false;
	var name = io_strip(nam).trim().toLowerCase();
	for(var i=0;i<io_team_arr.length;i++){
		if(io_team_arr[i]['tit'].trim().toLowerCase() == name){
			out = i;
		}
	}
	return out;
...
function io_get_member_link(nam){
	var out = nam;
	var member = io_get_member(nam);
	if(member){
		out = '<a href="/team/'+io_team_arr[member]['key']+'">'+io_team_arr[member]['tit']+'</a>';
	}
	return out;
}
	...
	if(dataload.data.creators != undefined){
		for(var c=0;c<dataload.data.creators.length;c++){
			...
			if(dataload.data.creators[c].firstName != undefined){
				creators += io_get_member_link(dataload.data.creators[c].firstName.split(' ').join('&nbsp;')+'&nbsp;'+dataload.data.creators[c].lastName.split(' ').join('&nbsp;'));
			}else{
				creators += io_get_member_link(dataload.data.creators[c].name);
			}
		}
	}
	return creators;
