...
var library_item_tools = '<a data-toggle="modal" href="#submit_paper" class="btn btn-default">Submit paper <em class="fa fa-paper-plane"></em></a>';
...
function io_zotero_item_selected(dataload) {
	...
	if(dataload.data.abstractNote != undefined && dataload.data.abstractNote != ''){
		...
		abstractNote += ')" class="block button_posts"><em class="fa fa-angle-double-down shower"></em><em class="fa fa-angle-double-up hider"></em></a></div>';
	}
	var place = '';
	...
	var out = '<button class="close"><em class="fa fa-times-circle"></em></button><hr />\
	...
	if(io_zotero_arr_obj[io_zotero_selection].icons == undefined){
		jQuery.getJSON('https://api.zotero.org/groups/'+io_zotero_group+'/items/'+io_zotero_selection+'/children?key='+io_zotero_pubkey+'&format=json&include=data', function (data) {
	    	if(data){
    			...
    			jQuery("#zotero_load_detail").html('<div class=""><h1 class="margint0">'+io_zotero_arr_obj[io_zotero_selection].data.title+'</h1></div>'+iframes).removeClass('none');
        ...
        <div class="text">\
            <div class="open_book" id="book-'+dataload.key+'" rel="'+dataload.key+'" onclick="io_zotero_item_click(this)">\
            	...
            	<div class="bg">\
                    ...
                    <div class="typeicon"><button class="close"><em class="fa fa-times-circle"></em></button>'+io_zotero_itemtype_icon[dataload.data.itemType]+'</div>\
...
function io_zotero_lib_list_setup() {
	...
	if(!tag){
    	...
    	jQuery("#zotero_load").html('<div class="col-lg-offset-3"><h2 class="margint0"><em class="icon fa fa-tag website-'+tag+' rounded"></em> '+tag+'</h2></div>'+io_zotero_library());
	...
	if(out != ''){
		if(io_zotero_search_autocomplete){
			jQuery(".io_autocomplete").html('<ul>'+out+'</ul>');
	   		...
	   		jQuery("#zotero_load_detail").empty();
			jQuery(".io_autocomplete").empty();
	    	jQuery("#zotero_load").html('<div class="col-lg-offset-2"><h2 class="margint0"><em class="icon fa fa-search searched rounded"></em> Search results for "'+searched+'"</h2></div> <ul class="results">'+out+'</ul>');
		}
	}
}
