...
function io_zotero_item_back_button() {
	if(io_zotero_arr_obj.length > 1){
		...
		document.location.href = '/research/library/';
	}
}
	...
	if(searched.length > 0){
		...
		for (var key in io_zotero_search_arr_obj) {
			if(io_zotero_search_arr_obj[key].data.title != undefined){ ///io_zotero_item_creators(dataload)
				...
				var tags = '';
				var tagline = '';
				var searching_title = io_zotero_search_arr_obj[key].data.title.toLowerCase().search(searched.toLowerCase());
				var searching_authors = io_strip(io_zotero_item_creators(io_zotero_search_arr_obj[key])).toLowerCase().search(searched.toLowerCase());
				...
				if(io_zotero_tag_ok(io_zotero_search_arr_obj[key].data.tags)){
					for(var i=0;i<io_zotero_search_arr_obj[key].data.tags.length;i++){
						if (io_zotero_search_arr_obj[key].data.tags[i].tag.match('website-')) {
							searching_tags[i] = io_zotero_search_arr_obj[key].data.tags[i].tag.toLowerCase().search(searched.toLowerCase());
				...
				if(str){
					...
					out += '<li class="item">';
					if(io_zotero_search_autocomplete){
						out += '<a href="#'+io_zotero_search_arr_obj[key].key+'">';
					}
					out += tit;
