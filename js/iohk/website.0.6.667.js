...
function io_loadmodal(obj,tit){
	$("#morecontent-modal-header").text(tit);
	$("#morecontent-modal-body").html(jQuery(obj).html());
}

var io_current_team_filter = 'show-all';
function io_team_filter(filt,obj){
	$(".filters .active").removeClass('active');
	$(".filters ."+obj).addClass('active');

	$("#teamgrid").removeClass(io_current_team_filter);
	io_current_team_filter = obj;
	$("#teamgrid").addClass(io_current_team_filter);

	$("#teamgrid .griditem").not('.'+filt).hide(500);
	if(filt == 'all'){
		$("#teamgrid .griditem").show(500);
	}else{
		filt = '.'+filt;
		$("#teamgrid "+filt).show(500);
	}
}
