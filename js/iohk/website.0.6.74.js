	...
	function io_which_way_alter(){
		...
		if($(".iohk-video-list").hasClass('youtube')){
		 	...
		 	$(".iohk-video-list").each(function(){
				iohk_playlist = $(this).attr('rel');
				iohk_playlist_div = $(this).attr('id');
				$("#"+iohk_playlist_div).html(svgloader);
				if($(this).attr('grid')){
					iohk_playlist_grid = $(this).attr('grid');
				}
				if($(this).attr('count')){
					iohk_playlist_count = $(this).attr('count')*1;
				}
